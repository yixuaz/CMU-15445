...
void BPLUSTREE_TYPE::Remove(const KeyType &key, Transaction *transaction) {
  ...
  if (curSize < delTar->GetMinSize()) {
    removeSuc = CoalesceOrRedistribute(delTar,transaction);
  }
  if (!removeSuc)
...
bool BPLUSTREE_TYPE::CoalesceOrRedistribute(N *node, Transaction *transaction) {
  ...
  N *node2;
  bool isRightSib = FindLeftSibling(node,node2);
  ...
  if (node->GetSize() + node2->GetSize() <= node->GetMaxSize()) {
    ...
    bool removeSuc = Coalesce(node2,node,parentPage,removeIndex,transaction);//unpin node,node2
    if (!removeSuc)
